...
PORT = "/dev/ttyUSB1"
BAUD = 115200
TICKS_PER_REV = 1260  # measured empirically (see rotate_test.py)
...
print("Forcing stop and confirming frozen")
send(0, 0)
drain_timed(2)

print("Sending full speed (0 1) for 20s, measuring real RPM")
send(0, 1)
samples = drain_timed(35)
...
if len(samples) >= 2:
    # Use the last 5s window as the steady-state plateau
    ...
    print(samples[::5]); print(f"Measured: {ticks_per_sec:.1f} ticks/sec = {rpm:.1f} RPM")
else:
